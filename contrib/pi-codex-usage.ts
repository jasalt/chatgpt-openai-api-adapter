...
export default function (pi: ExtensionAPI) {
	...
	const refresh = async (ctx: ExtensionContext, reportError = false): Promise<Status | undefined> => {
		const currentGeneration = ++generation;
		try {
			const status = await fetchStatus(ctx);
			if (currentGeneration !== generation) return undefined;
			ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("dim", formatStatusLine(status)));
			return status;
		} catch (error) {
	...
	pi.on("session_start", async (_event: unknown, ctx: ExtensionContext) => {
		await refresh(ctx);
		if (!timer) timer = setInterval(() => void refresh(ctx), REFRESH_MS);
	});
...
async function fetchStatus(ctx: ExtensionContext): Promise<Status> {
	...
	if (!auth?.auth.apiKey) throw new Error(`No authentication available for ${model.provider}`);
...
async function getJSON(url: string, headers: Record<string, string>): Promise<UsagePayload> {
	const response = await fetch(url, { headers, signal: AbortSignal.timeout(15_000) });
	if (!response.ok) {
		const body = (await response.text()).trim();
		throw new Error(`Codex usage request failed (HTTP ${response.status})${body ? `: ${body}` : ""}`);
	}
...
function parseStatus(payload: UsagePayload, account = payload.account): Status {
	...
	const weekly = [primary, secondary].find(
		(window) => window && Math.abs(window.limit_window_seconds - WEEK_SECONDS) <= WEEK_SECONDS * 0.05,
	);
	if (!primary && !weekly) throw new Error("ChatGPT did not return any Codex usage windows");
	return { account, primary, weekly };
}

function validWindow(value: Window | undefined): Window | undefined {
	return value && Number.isFinite(value.used_percent) && Number.isFinite(value.limit_window_seconds) &&
		Number.isFinite(value.reset_at) ? value : undefined;
}

function formatStatusLine(status: Status): string {
	return [
		status.primary && compactWindow(status.primary, durationLabel(status.primary)),
		status.weekly && status.weekly !== status.primary && compactWindow(status.weekly, "w"),
	].filter(Boolean).join("  ");
}
...
function formatStatusCard(status: Status): string {
	...
	if (status.account?.email) {
		const plan = status.account.plan ? ` (${titleWord(status.account.plan)})` : "";
		lines.push(`Account:       ${status.account.email}${plan}`);
	}
	if (status.primary) lines.push(`${durationLabel(status.primary)} limit:      ${fullWindow(status.primary)}`);
	if (status.weekly && status.weekly !== status.primary) lines.push(`Weekly limit: ${fullWindow(status.weekly)}`);
	return lines.join("\n");
}
...
function formatReset(window: Window, includeWeeklyDate: boolean): string {
	...
	if (!includeWeeklyDate || window.limit_window_seconds < 24 * 60 * 60) return time;
